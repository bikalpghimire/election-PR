...
def calculate_seats(df, total_seats, threshold_pct, manual_total_votes):
    ...
    return (
        eligible_df[["Party", "Votes", "Seats"]].sort_values(
...
class ElectionApp:
    def __init__(self, root):
        ...
        try:
            ...
            self.logo_img = ImageTk.PhotoImage(
                load.resize((180, 50), Image.Resampling.LANCZOS)
            )
        except Exception:
            self.logo_img = None
        ...
        style.configure(
            "Treeview.Heading", font=(FONT_NAME, 14, "bold"), background="#d1d8e0"
        )
        ...
        tk.Label(
            header,
            text="Nepal Election PR Seat Calculator",
        ...
        tk.Label(
            header,
            text="Developed By: Pumori Engineering Services",
            font=(FONT_NAME, 12),
            fg="#bdc3c7",
            bg="#2c3e50",
        ).place(relx=1.0, rely=0.5, anchor="e", x=-30)
        ...
        tk.Button(
            ctrl_bar,
            text="🌐 Load From Web",
            command=self.fetch_from_web,
            bg="#16a085",
            **btn_config,
        ).grid(row=0, column=1, padx=10)
        self.seats_entry = tk.Entry(
            ctrl_bar,
            width=8,
            font=(FONT_NAME, 12),
            justify="center",
        ...
        self.main_container = tk.PanedWindow(
            root, orient="horizontal", bg="#f8f9fa", sashwidth=6
        )
        ...
        self.tree = ttk.Treeview(
            self.table_frame, columns=("Party", "Votes", "Seats"), show="headings"
        )
        self.tree.heading("Party", text="Political Party Name")
        self.tree.column("Party", width=280, anchor="w")
        self.tree.heading("Votes", text="Votes")
        self.tree.column("Votes", width=120, anchor="center")
        self.tree.heading("Seats", text="Seats")
        self.tree.column("Seats", width=100, anchor="center")
        self.tree.pack(fill="both", expand=True)

        self.right_pane = tk.PanedWindow(
            self.main_container, orient="vertical", bg="#f8f9fa", sashwidth=6
        )
        ...
        self.lbl_total_votes = tk.Label(
            self.summary_frame, text="Total Valid Votes: -", **lbl_style
        )
        self.lbl_total_votes.pack(side="left", padx=40)
        self.lbl_threshold = tk.Label(
            self.summary_frame, text="3% Threshold: -", **lbl_style
        )
        self.lbl_threshold.pack(side="left", padx=40)
        self.lbl_qualified = tk.Label(
            self.summary_frame, text="Qualified Parties: -", **lbl_style
        )
        self.lbl_qualified.pack(side="left", padx=40)
    ...
    def fetch_from_web(self):
        ...
        try:
            ...
            self.logo_map = dict(zip(df["Party"], df["Logo"]))

            self.total_input_votes = total_votes

            res, total, thresh, count, total_p = calculate_seats(
                df[["Party", "Votes"]], int(self.seats_entry.get()), 3.0, total_votes
            )

            # merge logos into results
            if res is not None:
            ...
            self.lbl_qualified.config(
                text=f"Qualified Parties: {count} out of {total_p}"
            )
    ...
    def update_chart(self):
        ...
        for wedge, (_, row) in zip(wedges, chart_data.iterrows()):
            ...
            try:
                ...
                if logo_url not in self.logo_cache:
                    ...
                    self.logo_cache[logo_url] = np.array(img)

                img = self.logo_cache[logo_url]
        ...
        fig.suptitle("PR Seat Allocation Result", fontsize=12, y=0.99)
        ax.axis("equal")
        ...
        def on_scroll(event):
            ...
            new_width, new_height = (cur_xlim[1] - cur_xlim[0]) * scale_factor, (
                cur_ylim[1] - cur_ylim[0]
            ) * scale_factor
            rel_x, rel_y = (cur_xlim[1] - event.xdata) / (cur_xlim[1] - cur_xlim[0]), (
                cur_ylim[1] - event.ydata
            ) / (cur_ylim[1] - cur_ylim[0])
            ax.set_xlim(
                [event.xdata - new_width * (1 - rel_x), event.xdata + new_width * rel_x]
            )
            ax.set_ylim(
                [
        ...
        fig.canvas.mpl_connect(
            "button_release_event", lambda e: setattr(self, "pan_start", None)
        )
    ...
    def process(self):
        ...
        try:
            ...
            for i in range(1, len(df_clean) - 1, 2):
                p_name, p_votes = str(df_clean.iloc[i, 0]).strip(), self.clean_number(
                    df_clean.iloc[i + 1, 0]
                )
            ...
            res, total, thresh, count, total_p = calculate_seats(
                pd.DataFrame(party_data), int(self.seats_entry.get()), 3.0, manual_total
            )
            self.result_df = res
            self.lbl_total_votes.config(text=f"Total Valid Votes: {int(total):,}")
            self.lbl_threshold.config(text=f"3% Threshold: {int(thresh):,}")
    ...
    def reset_app(self):
        self.filepath = self.result_df = None
        self.total_input_votes = 0
        for i in self.tree_qualified.get_children():
            self.tree_qualified.delete(i)
        for w in self.chart_frame.winfo_children():
        ...
        self.lbl_qualified.config(text="Qualified Parties: -")
        winsound.MessageBeep(winsound.MB_ICONASTERISK)

    def export_file(self):
        if self.result_df is not None:
            path = filedialog.asksaveasfilename(
                initialfile="Nepal_PR_result.xlsx", defaultextension=".xlsx"
            )
            if path:
                ...
                worksheet.add_table(
                    ...
                    max_row,
                    max_col - 1,
                    {"columns": columns, "style": "Table Style Medium 9"},
                )
                for i, col in enumerate(self.result_df.columns):
                    width = (
                        max(self.result_df[col].astype(str).str.len().max(), len(col))
                        + 2
                    )
...
if __name__ == "__main__":
    try:
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(
            "pumori.engineering.seatcalc.pro"
        )
    except Exception:
        pass
    root = tk.Tk()
